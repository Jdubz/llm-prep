# Module 05: Advanced Load Balancing Patterns
...
**Connection pool sizing formula:** `pool_size >= peak_rps * avg_response_time_seconds`
...
1. **How would you design a global load balancing architecture for a service that needs sub-100ms latency worldwide?**

   Layer the solution: GeoDNS or anycast routing at the DNS layer to direct users to the nearest region. An L4 load balancer (NLB/MetalLB) at each region for raw throughput. L7 load balancers (Envoy) behind that for content-based routing. CDN at the edge for static assets and cacheable API responses. For dynamic content, deploy the service in multiple regions with a replicated data layer (CockroachDB, DynamoDB Global Tables). Use latency-based routing in DNS rather than pure geo -- sometimes a geographically farther region has lower latency due to network topology.

2. **Explain why consistent hashing is important for cache-aware load balancing. What happens when you add a node?**
...
3. **Design the load balancing layer for a real-time multiplayer game.**
