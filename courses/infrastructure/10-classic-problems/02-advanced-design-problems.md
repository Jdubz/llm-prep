# Module 10: Advanced System Design Problems
...
## Problem 7: Distributed Task Scheduler

### Requirements

**Functional:** Schedule tasks for one-time or recurring execution (cron-like). Support priority queues (critical tasks run before low-priority ones). Exactly-once execution guarantee. Failure recovery (retry failed tasks with backoff). Task dependencies (task B runs after task A completes). Multi-datacenter support.

**Non-Functional:** Scale: 10M scheduled tasks, 100K executions/day. Latency: tasks execute within 1 second of their scheduled time. Durability: scheduled tasks survive node failures.
...
**Approach 3: Idempotent Execution.** Design task handlers to be idempotent -- even if a task runs twice, the second execution produces no side effects. Use idempotency keys with external services.
...
Alternatively, use a single queue with priority scoring via Redis Sorted Sets:
...
State machine: `PENDING -> CLAIMED -> RUNNING -> COMPLETED` or `RUNNING -> FAILED -> RETRY (with exponential backoff) -> CLAIMED` or `FAILED -> DEAD_LETTER (after max retries)`.

A sweeper process detects stuck tasks:

```sql
-- Find tasks running longer than 30 minutes (likely crashed)
...
A cron engine evaluates cron expressions every minute and creates task instances at scheduled times. Deduplication: unique constraint on `(cron_id, scheduled_time)` prevents duplicates even if multiple engine instances are running.

### Deep Dive: Multi-Datacenter

Tasks may need to execute in a specific region for data locality or compliance. Use regional queues (`task_queue_us_east`, `task_queue_eu_west`) with a router that inspects `task.region` and enqueues to the correct regional queue. Workers consume only from their local queue. If a region goes down, tasks re-route to a failover region.

### Architecture Summary

API Layer (submit/cancel/query) + Scheduler (poll due tasks) + Cron Engine (create instances) + Sweeper (detect stuck tasks) -> Task Store (PostgreSQL) -> Priority Queues (Redis/SQS: high, normal, low) -> Worker Pool (execute, report status, retry).
...
**Functional:** As the user types, suggest completions based on prefix matching. Rank suggestions by frequency (most popular queries first). Support personalization (recent searches, user-specific suggestions). Update suggestions in near-real-time as search trends change. Return top 10 suggestions within 100ms.

**Non-Functional:** Scale: 10B queries/day, 100K unique queries added/day. Latency: < 50ms p99 (autocomplete must feel instant). Availability: high (degraded autocomplete is acceptable; no autocomplete is not).
...
**Optimization 1: Store top-K at each node.** Instead of traversing all descendants, pre-compute and cache the top 10 at each node. Makes query-time lookups O(prefix_length) instead of O(subtree_size).

**Optimization 2: Compressed trie (Patricia trie).** Merge single-child chains into a single node. "h-o-t-e-l" becomes a single edge "hotel". Reduces memory by 50-70%.
...
# score = sum(count_i * decay^(now - time_i)), where decay = 0.95 (per hour)
```
...
**Tier 1: Batch-built trie (rebuilt every few hours).** Covers the stable, long-term query distribution. Stored in a read-optimized format (memory-mapped file or distributed cache).

**Tier 2: Real-time trending overlay.** A small data structure that captures trending queries from the last few minutes/hours. Implemented with a streaming counter (Count-Min Sketch or Redis sorted set).
...
│ │ (real-time, small) │<───│ │ (Count-Min Sketch or  │         │
│ └────────────────────┘    │ │  Redis sorted set)    │         │
│   |                       │ └───────────────────────┘         │
...
## Problem 9: Video Streaming Platform

### Requirements

**Functional:** Upload videos (up to 10 GB). Transcode videos into multiple resolutions and formats. Stream video with adaptive bitrate (quality adjusts to network conditions). Support live streaming. Recommendation feed (personalized). Comments, likes, view counts.

**Non-Functional:** Scale: 1B video views/day, 100K uploads/day. Latency: video playback starts within 2 seconds. Availability: 99.99% for playback. Storage: petabytes of video content.
...
5. If bandwidth increased, switches up (720p -> 1080p). If decreased, switches down (720p -> 480p).
6. Switches happen at segment boundaries (every 4 seconds) -- seamless to the viewer.
...
**Cache strategy:** Popular videos (top 10% by views) are pre-warmed at edge PoPs. Long-tail videos are cached on demand. Early segments within a video are cached more aggressively (many viewers start but few finish).
...
**Low-latency HLS (LL-HLS):** Uses partial segments (0.5-1 second chunks within a 4-second segment). Players fetch partial segments as they are produced. Reduces latency from 15-30 seconds to 3-5 seconds. Requires CDN support for chunked transfer encoding.

### Deep Dive: Recommendation Feed

```
User opens app -> Recommendation Service
  -> Candidate generation (retrieve 1000 candidates):
...
## Extended Problem-Specific Key Decisions
...
| **Task Scheduler** | Priority | Separate queues vs. sorted set | Number of priority levels, fairness needs |
| **Autocomplete** | Data structure | Trie (in-memory) vs. search index (Elasticsearch) | Latency requirement, update frequency |
| **Autocomplete** | Updates | Batch rebuild vs. real-time overlay | Freshness vs. consistency trade-off |
