# Module 04: Message Queue Operations and Patterns
...
## Message Broker Extended Comparison
...
1. **Messages piling up in Kafka. Walk through debugging.**

   Is consumer running? Rebalancing loops? Throughput? Poison messages? GC? Partition count vs consumer count?

2. **50K messages in SQS DLQ. Investigate and recover.**
...
4. **How do you guarantee atomic write to database and event publish without distributed transactions?**

   Outbox pattern: write outbox row in same transaction as business data. Separate publisher reads outbox and publishes to Kafka, marks as published. Or use Debezium to capture outbox table changes via CDC.
